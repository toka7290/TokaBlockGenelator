      ...
      $("<div>")
        ...
        .append(
          $("<label>").append(
            $("<div>")
              .addClass("array-num")
              .text(array_list.children().length),
            $("<input>")
 ...
 */
function addEvent() {
  $(".event.status-block-body").append(
    $("#event-status-block")
      .contents()
      .clone()
  );
...
function addBlockState() {
  $(".blockState.status-block-body").append(
...
$(".file-drop-zone,.file-drop-zone-textarea").on("dragleave", function(/** @type {Event} */ event) {
  event.stopPropagation();
  event.preventDefault();
...
function onChangedFlammable() {
  ...
  for (let index = 0; index < flame_odds_len; index++) {
    ...
    burn_odds
      .eq(index)
      .closest(".value-element")
      .toggleClass("disabled", is_empty);
  }
}
/** イシューチェック */
function checkIssue() {
      ...
      switch (typeof states[0]) {
        case "number":
          ...
          for (let states_index = 0; states_index < states.length; states_index++) {
            ...
            block_content
              .find(".blockState-integer")
              .eq(states_index)
              .val(states[states_index]);
          }
          break;
        case "string":
  ...
  if (json_data["minecraft:block"]["permutations"] != null) {
    ...
    for (let index = 0; index < permutations.length; index++) {
      ...
      if (permutations[index]["condition"] != null) {
        $(".permutations_condition")
          .eq(index)
          .val(permutations[index]["condition"]);
      }
      setComponents(
        $(".permutations.block-tab-container")
          .eq(index)
          .find(".editor-element:last-child"),
        format_version,
    ...
    case "1.16.100":
      ...
      if (is_exist) {
        ...
        for (let index = 0; index < key.length; index++) {
          ...
          inner_element
            .find(".components-material-instances-face")
            .eq(index)
            .val(key_name);
...
function setEventResponses(element_scope, json_data) {
  ...
  if (is_exist) {
    ...
    for (let index = 0; index < keys.length; index++) {
      ...
      const key = keys[index];
      inner_element
        .find(".event-responses-set-block-state")
        .eq(index)
        .val(key);
      inner_element
  ...
  if (is_exist) {
    ...
    if (inner_json["command"] != null) {
      ...
      for (let index = 0; index < commands.length; index++) {
        ...
        inner_element
          .find(".event-responses-run-command")
          .eq(index)
          .val(inner_json["command"]);
      }
    }
    if (inner_json["target"] != null)
